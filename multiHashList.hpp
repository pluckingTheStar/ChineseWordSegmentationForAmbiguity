...
class multiHashList{
    ...
    void info(){
        ...
        int totalLen = 0;
        for (int i = 0; i < len; i ++){
            cout << "table[" << i << "] Length:" << tableLen[i];
            totalLen += tableLen[i];
            countTable = 0;
            ...
            cout << " LoadingPercentage: " << float(countTable) / float(tableLen[i]) << endl;
        }
        cout << "loaded: " << countTables << "/" << totalLen << " LoadingPercentage: " << float(countTables) / float(totalLen) << endl;
        return;
    }
    ~multiHashList(){
